    ...
    return (
        <div ref={ref} className="flex flex-row ml-6 gap-6 mb-2">
            ...
            <div className="relative">
                <span className="text-white text-sm bg-black py-1 px-2 rounded-2xl absolute top-1/2 transform -translate-y-1/2 -right-32 opacity-0 hover:opacity-100">
                    {message.date.toDate().toLocaleTimeString()}
                </span>
                {message.text !== '' ? (
                    <p className=" inline-block break-words max-w-xs text-white bg-gray-500 px-4 py-2 rounded-xl">
                        {message.text}
                    </p>
                ) : (
                    <span></span>
